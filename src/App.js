...
function loadData() { try { const d = localStorage.getItem("taskapp_v5"); return d ? JSON.parse(d) : null; } catch { return null; } }
function saveData(d) { try { localStorage.setItem("taskapp_v5", JSON.stringify(d)); } catch {} }
...
export default function App() {
  ...
  function deleteTask(id) { setTasks(prev => prev.filter(t => t.id !== id)); showToast("🗑️ Task deleted!"); }
  ...
  function endDay() {
    ...
    showToast(`🏁 Day ended! ${done}/${todayT.length} complete!`, "appreciate");
  }

  function exportCSV() {
    const headers = ["Title","Description","Priority","Date","Time","Status"];
    const rows = tasks.map(t => [`"${t.title}"`,`"${t.desc||""}"`,t.priority,t.date,t.time||"",t.done?"Completed":"Pending"]);
  ...
  function exportPDF() {
    const today = getToday();
    const html = `<html><head><title>Task Report</title><style>body{font-family:Arial;padding:24px;} h1{color:#6366f1;} table{width:100%;border-collapse:collapse;} th{background:#6366f1;color:#fff;padding:8px;text-align:left;} td{padding:8px;border-bottom:1px solid #eee;}</style></head><body><h1>📋 Daily Task Tracker</h1><p>Date: ${today} | Total: ${tasks.length} | Done: ${tasks.filter(t=>t.done).length}</p><table><tr><th>#</th><th>Task</th><th>Priority</th><th>Date</th><th>Status</th></tr>${tasks.map((t,i)=>`<tr><td>${i+1}</td><td>${t.title}</td><td>${t.priority}</td><td>${t.date}</td><td>${t.done?"✅":"⏳"}</td></tr>`).join("")}</table></body></html>`;
          ...
          {view==="dashboard" && (<>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1.2fr 0.8fr 1fr", gap:13 }}>
              <div style={{ background:c.card, borderRadius:13, padding:"14px", border:`1px solid ${c.border}` }}>
                ...
                {todayTasks.length>0&&(<button onClick={endDay} style={{ width:"100%", marginTop:9, background:`linear-gradient(90deg,${c.yellow},#f97316)`, border:"none", borderRadius:9, color:"#fff", padding:"8px", fontWeight:700, fontSize:13, cursor:"pointer" }}>🏁 Day End & Save Achievement</button>)}
              </div>
              <div style={{ background:c.card, borderRadius:13, padding:"14px", border:`1px solid ${c.border}` }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:320, overflowY:"auto" }}>
                  {[...tasks].reverse().slice(0,12).map((t,i)=>(
                    <div key={i} style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                      ...
                      <div><div style={{ fontSize:12, fontWeight:600 }}>{t.title}</div><div style={{ fontSize:10, color:c.sub }}>{t.done?"Completed":"Pending"} · {t.date}</div></div>
                    </div>
                  ))}
              ...
              <div style={{ background:c.card, borderRadius:13, padding:"14px", border:`1px solid ${c.border}` }}>
                ...
                {[{label:"Completed",value:allDone,color:c.green,icon:"✅"},{label:"Pending Today",value:pendingTasks.length,color:c.yellow,icon:"⏳"},{label:"Streak",value:`${streak} days`,color:c.accent,icon:"🔥"}].map((s,i)=>(
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {[...history].reverse().map((h,i)=>(
                  <div key={i} style={{ padding:"10px 13px", background:c.card2, borderRadius:10, borderLeft:`4px solid ${h.type==="daily_summary"?c.accent:c.yellow}` }}>
                    {h.type==="daily_summary"?(<><div style={{ fontWeight:700, color:c.accent, fontSize:13 }}>📅 {h.date} — Daily Summary</div><div style={{ fontSize:19, fontWeight:800, marginTop:3 }}>{h.achievement}</div><div style={{ fontSize:12, color:c.sub, marginTop:2 }}>{h.done}/{h.total} Tasks Complete</div></>):(<><div style={{ fontWeight:600, fontSize:13 }}>🔄 {h.title}</div><div style={{ fontSize:11, color:c.sub, marginTop:2 }}>{h.date} → {h.rolledTo}</div><div style={{ fontSize:11, color:c.yellow, marginTop:2 }}>Reason: {h.reason}</div></>)}
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ position:"fixed", bottom:24, right:24, zIndex:50 }}>
        {fabOpen&&(
          <div style={{ position:"absolute", bottom:60, right:0, display:"flex", flexDirection:"column", gap:6, alignItems:"flex-end" }}>
            {[{label:"Add Task",icon:"➕",action:()=>{setShowAdd(true);setFabOpen(false);}},{label:"Day End",icon:"🏁",action:()=>{endDay();setFabOpen(false);}}].map((f,i)=>(
              <button key={i} onClick={f.action} style={{ display:"flex", alignItems:"center", gap:8, background:c.card, border:`1px solid ${c.border}`, borderRadius:10, padding:"7px 14px", color:c.text, fontSize:13, fontWeight:600, cursor:"pointer", boxShadow:"0 4px 16px #0006", whiteSpace:"nowrap" }}>{f.icon} {f.label}</button>
            ))}
          </div>
        )}
      ...
      {editModal&&(
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }} onClick={()=>setEditModal(null)}>
          <div style={{ background:c.card, borderRadius:17, padding:24, width:410, border:`1px solid ${c.border}` }} onClick={e=>e.stopPropagation()}>
            ...
            {[{ph:"Task name *",key:"title"},{ph:"Description (optional)",key:"desc"}].map(f=>(<input key={f.key} style={{ width:"100%", background:darkMode?"#0f1117":c.card2, border:`1px solid ${c.border}`, borderRadius:8, padding:"9px 12px", color:c.text, fontSize:14, boxSizing:"border-box", marginBottom:10, outline:"none" }} placeholder={f.ph} value={editForm[f.key]} onChange={e=>setEditForm(p=>({...p,[f.key]:e.target.value}))} />))}
